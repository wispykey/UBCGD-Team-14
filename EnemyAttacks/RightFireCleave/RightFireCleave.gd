...
func _ready() -> void:
	...
	for i in dimensions.x:
		for j in dimensions.y:
			...
			telegraph.position.x = TILE_SIZE*(i + 0.5)
			telegraph.position.y = TILE_SIZE*(j - floor(dimensions.y/2))
...
func _on_telegraph_timer_timeout():
	...
	for i in dimensions.x:
		for j in dimensions.y:
			...
			attack.position.x = TILE_SIZE*(i + 0.5)
			attack.position.y = TILE_SIZE*(j - floor(dimensions.y/2))
			add_child(attack)
	$DespawnTimer.start()
